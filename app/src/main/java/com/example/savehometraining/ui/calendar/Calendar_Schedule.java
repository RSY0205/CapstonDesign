    ...
    protected void onCreate(Bundle saveInstanceState) {
        ...
        listview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                rehabilitation.putExtra("min",Schedule.get(position).getCalendar_min());
                rehabilitation.putExtra("sec",Schedule.get(position).getCalendar_sec());
            ...
            public void onResponse(Call<LoadDayInfoResponse> call, Response<LoadDayInfoResponse> response) {
                ...
                if(result.getResult()=="true"){
                    ...
                    for(int i=0;i<result.getResponse().size();i++){
                        String context="";
                        if(result.getResponse().get(i).getCalendar_title().equals("플랭크")||result.getResponse().get(i).getCalendar_title().equals("홈사이클")||result.getResponse().get(i).getCalendar_title().equals("러닝")) {
                            context =(result.getResponse().get(i).getCalendar_title()+" : "+result.getResponse().get(i).getCalendar_min()+"분"+result.getResponse().get(i).getCalendar_sec()+"초\n메모 : "+result.getResponse().get(i).getCalendar_memo());
                        }
                        else
                        {
                            context = (result.getResponse().get(i).getCalendar_title()+" : "+result.getResponse().get(i).getCalendar_count()+"개\n메모 : "+result.getResponse().get(i).getCalendar_memo());
                        }
